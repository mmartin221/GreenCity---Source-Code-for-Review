...
<section class="ls with_bottom_border">
			...
			<section class="ls section_padding_top_50 section_padding_bottom_50 columns_padding_10">
				<div class="container-fluid">
					...
					<form class="form-horizontal" method="post" enctype='multipart/form-data' action="<?php echo base_url(); ?>edit-product/<?= $result['id']; ?>">
							...
							<div class="col-md-9">
								<div class="with_border with_padding">
										...
										<div class="col-lg-9">
											<select class="form-control" name="category" required="">
												...
												<?php foreach ($category as $key => $value) { ?>
													<option value="<?= $value['id']; ?>" <?php if($result['categoryId'] == $value['id']) { echo 'selected'; } ?> ><?= $value['name']; ?></option>
												<?php } ?>
											</select>
										</div>
									</div>
									...
									<div class="row form-group admin-product-price">
										...
										<div class="col-lg-9">
											<input type="text" step="0.01" name="quantity" value="<?= $result['quantity']; ?>" class="form-control num_only" required>
										</div>
									</div>
									...
									<div class="row form-group">
										...
										<div class="col-lg-9">
											<textarea rows="3" name="description" class="form-control" ><?= $result['description']; ?></textarea>
										</div>
									</div>
									...
									<div class="row form-group">
										...
										<div class="col-lg-9">
											<textarea rows="9" name="descriptionLong" class="form-control" ><?= $result['descriptionLong']; ?></textarea>
										</div>
									</div>
									...
									<div class="row form-group">
										...
										<div class="col-lg-9">
											<input type="file" class="form-control" id="pro-image" name="image[]" onclick="$('#pro-image').click()" multiple  style="padding-top: 10px; padding-bottom: 10px;">
											<div class="preview-images-zone">
								                <?php $i = 100; foreach ($images as $key => $value) { ?>
								                    <div class="preview-image preview-show-<?= $i; ?>">
								                        <div class="image-cancel" onclick="deleteImage('<?= $value['id']; ?>', '<?= $i; ?>')">x</div>
								                        <div class="image-zone"><img id="pro-img-<?= $i; ?>" src="<?php echo base_url(); ?>uploads/<?= $value['image']; ?>"></div>
								                    </div>
								                <?php $i++; } ?>
								            </div>
										</div>
